...
// VITE_GITHUB_TOKEN (optional, no scopes) raises the GitHub API limit from 60/hr to 5,000/hr.
// Set it in .env locally and as an env var on Cloudflare Pages — see README.
const GITHUB_TOKEN = import.meta.env.VITE_GITHUB_TOKEN;
const GH_HEADERS = GITHUB_TOKEN ? { Authorization: `Bearer ${GITHUB_TOKEN}` } : {};
...
const LANG_COLORS = {
  ...
  CSS: '#563d7c', Jupyter: '#DA5B0B', Java: '#b07219', 'C++': '#f34b7d',
...
function LanguageBar({ languages }) {
  ...
  return (
    <div className="mb-4 w-full max-w-full min-w-0">
      <div className="flex h-1.5 rounded-full overflow-hidden bg-line mb-2 w-full max-w-full">
        {withPct.map((l) => (
          <motion.div
            key={l.name}
            initial={{ width: 0 }}
            ...
            style={{ background: colorForLang(l.name) }}
            title={`${l.name} ${l.pct.toFixed(1)}%`}
          />
        ))}
      </div>
      <div className="flex flex-wrap gap-x-3 gap-y-1 max-w-full">
        {shown.map((l) => (
        ...
        {withPct.length > 4 && (
          <span className="font-mono text-[10px] text-dim/60">+{withPct.length - 4} more</span>
        )}
      </div>
    </div>
  );
...
export default function GitHubProjects() {
      ...
      toFetch.map(async (repo) => {
        const [readmeRes, langRes] = await Promise.allSettled([
          fetch(`https://api.github.com/repos/${GITHUB_USERNAME}/${repo.name}/readme`, {
            headers: { Accept: 'application/vnd.github.v3.raw', ...GH_HEADERS },
          }),
          fetch(`https://api.github.com/repos/${GITHUB_USERNAME}/${repo.name}/languages`, { headers: GH_HEADERS }),
        ]);
  ...
  return (
    <section id="github" className="section-pad !pt-0 w-full max-w-full min-w-0 ">
      <Reveal className="w-full max-w-full min-w-0">
        <div className="flex items-center justify-between gap-4 mb-8 w-full max-w-full min-w-0">
          <p className="font-mono text-dim text-xs tracking-widest flex items-center gap-2 min-w-0">
            <span className="text-dim/50">03</span>
            <Github size={14} className="shrink-0" />
            <span className="truncate">LIVE FROM GITHUB</span>
          </p>
          
          < a  href={`https://github.com/${GITHUB_USERNAME}`}
            target="_blank"
            rel="noreferrer"
            className="font-mono text-xs text-signal hover:underline flex items-center gap-1 shrink-0"
          >
            View all
      ...
      {error && (
        <p className="text-dim text-sm font-mono break-words">
          Couldn't load repositories right now — check back later.
        </p>
      )}
      ...
      {!error && !allRepos && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 w-full max-w-full min-w-0">
          {[1, 2].map((i) => (
            <div key={i}  className="border border-line rounded-md p-7 h-48 animate-pulse bg-panel/30 w-full max-w-full min-w-0" />
          ))}
        </div>
      )}
      ...
      {allRepos && allRepos.length > 0 && (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 min-h-[300px] w-full max-w-full min-w-0">
            {pageRepos.map((repo, i) => {
              const details = detailsCache[repo.name];
              const isLoading = !details;

              return (
                <Reveal key={repo.id} delay={i * 0.06} className="w-full max-w-full min-w-0">
                  {isLoading ? (
                    <div className="border border-line rounded-md p-7 md:p-8 h-full w-full max-w-full min-w-0 bg-panel/30 animate-pulse min-h-[240px]" />
                  ) : (
                    <motion.a
                      href={repo.html_url}
                      target="_blank"
                     ...
                     className="group flex flex-col w-full max-w-full min-w-0 h-full border border-line/50 rounded-md p-6 md:p-8 bg-panel/25 hover:border-signal/40 hover:bg-panel/60 transition-colors overflow-hidden"
                    >
                      <div className="flex items-start justify-between gap-3 mb-1 w-full max-w-full min-w-0">
                        <h3 className="font-display text-2xl font-semibold text-ink break-all min-w-0 max-w-full">
                          {repo.name}
                        </h3>
                      ...
                      <div className="flex flex-wrap items-center gap-4 font-mono text-[11px] text-dim mt-auto">
          ...
          {totalPages > 1 && (
            <div className="flex flex-wrap items-center justify-center gap-4 mt-8 font-mono text-xs text-dim">
              <button
                onClick={goPrev}
                disabled={page === 0}
